...
public class FollowController {
    ...
    /**
     * 사용자가 다른 사용자를 팔로우합니다.
     * @param jwtToken JWT 토큰을 통해 사용자 ID 추출
     * @param followingId 팔로우할 대상 사용자 ID
     * @return 팔로우 성공 여부 메시지
     */
    @PostMapping("/follow")
    public ResponseEntity<String> follow(@RequestHeader("Authorization") String jwtToken, @RequestParam Long followingId) {
        try {
            followService.follow(jwtToken, followingId);
            log.info("팔로우 성공: followingId = {}", followingId);
    ...
    public ResponseEntity<String> unfollow(@RequestHeader("Authorization") String jwtToken, @RequestParam Long followingId) {
        try {
            followService.unfollow(jwtToken, followingId);
            log.info("언팔로우 성공: followingId = {}", followingId);
